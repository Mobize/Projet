...
$resul=$pdo->prepare('SELECT phot.url,ann.description_longue,ann.date_enregistrement,ann.prix,ann.adresse,ann.cp,ann.ville,ann.titre,ann.id_annonce,mem.pseudo,note.note
FROM photo phot,
annonce ann,
membre mem,
note note
...
if ( isset($_POST['submit'] ) && !empty($_POST['avis']) && !empty($_POST['note']))
{
    if(strlen($_POST['note']== 1))
    {
        if(is_numeric($_POST['note']))
        {
            ...
            //$insert=$pdo->prepare("INSERT INTO note VALUES($_SESSION['membre']['id_membre'],$id_vendeur,$_POST['note'],$_POST['avis'])");
            //$insert=$pdo->prepare("INSERT INTO note(membre_id1,membre_id2,note,avis) VALUES(1,1,5,'blabla')");
